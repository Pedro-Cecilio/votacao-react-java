...
class PautaControllerTest {
    ...
    public PautaControllerTest(AutenticacaoRepository autenticacaoRepository,
            UsuarioRepository usuarioRepository, MockMvc mockMvc, JacksonTester<CriarPautaDto> criarPautaDtoJson,
    ...
    void givenNaoEnvioCategoriaWhenBuscoTodasMinhasPautasWhenRetornarListaDePautas() throws Exception {
        Pauta pautaSaude = new Pauta("Você está bem de saúde?", Categoria.SAUDE.toString(), this.usuarioCadastrado);
        Pauta pautaTransporte = new Pauta("Sabe dirigir?", Categoria.TRANSPORTE.toString(), this.usuarioCadastrado);
    ...
    void givenPossuoPautaIdWhenBuscoAtivaPorIdWhenRetornarRespostaPautaDto() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders
                ...
                .andExpect(jsonPath("$.usuario.id").value(this.usuarioCadastrado.getId()))
                .andExpect(jsonPath("$.sessaoVotacao.id").value(pautaTransporte.getSessaoVotacao().getId()));       
    }
    @Test
    @DisplayName("Não deve ser possível buscar pauta ativa por Id ao passar id inválido ou de pauta não ativa")
